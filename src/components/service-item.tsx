 ...
 *
 * @param props.service - Service data object containing `name`, `description`, `icon` (Lucide icon name as a string), and optional `urlPath`.
...
export function ServiceItem({ href, service }: { href?: string; service: ItemWithIcon }) {
  ...
  const linkHref = href ?? service.urlPath;
  const content = (
    <>
      <Icon className="size-8 shrink-0" />
      <div>
        <div>
          <h3 className={`text-foreground block font-semibold ${linkHref ? 'underline' : ''}`}>{service.name}</h3>
          <p className="text-muted-foreground-2">{service.description}</p>
          {linkHref && (
            <span className="text-primary hover:text-primary-hover focus:text-primary-focus">Read more →</span>
          )}
        </div>
      </div>
    </>
